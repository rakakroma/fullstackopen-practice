    ...
    if (persons.find(person => person.name === newName)) {
      if (window.confirm(`${newName} is already added to phonebook, replace the old number with a new one?`)) {
        ...
        contactsAPI.update(persons[updateItemIndex].id, newObject)
          ...
          // })
          .catch(error => {
            if (error.response.data.error[0] === "ValidationError") {
              console.log("ValidationError");
              setMessage(['ERROR', error.response.data.error[1]])
            } else {
              console.log(error.response.data);
              setMessage(['ERROR', `can't update this contact info to server`])
            }
      ...
      setMessage(["ERROR", "Please fill the input with more than 3 characters"])
    } else if (!newNumber) {
      setMessage(['ERROR', 'Please fill the input'])
    } else {
      /// 由於id在資料庫端建立，我們又以id作為刪除之用，所以現在把新建立（未刷新前）的東西刪掉會失敗
